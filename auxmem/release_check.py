...
Run via ``python -m auxmem.release_check`` or ``bash scripts/check_release.sh``.
"""
...
from auxmem.upgrade import MANIFEST_SRC, TEMPLATE_DIR
...
def check_cli_version_consistency() -> CheckResult:
    ...
    if pyproject != __version__:
        return CheckResult(
            "cli-version",
            False,
            f"pyproject.toml ({pyproject}) != auxmem.__version__ ({__version__})",
        )
...
def check_generated_auxmem_ci() -> CheckResult:
    workflow = TEMPLATE_DIR / ".github" / "workflows" / "auxmem-check.yml"
    if not workflow.is_file():
        return CheckResult("auxmem-ci", False, "template workflow missing")
    text = workflow.read_text(encoding="utf-8")
    if "check_auxmem.py" not in text:
        return CheckResult("auxmem-ci", False, "workflow must run check_auxmem.py")
    return CheckResult("auxmem-ci", True)
...
def run_scaffold_smoke() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="auxmem-release-") as td:
        ...
        upgrade = run_auxmem(["upgrade", str(dest), "--dry-run"])
        ...
        if not workflow.is_file():
            raise ReleaseCheckError("scaffold missing generated auxmem-check.yml")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="AuxMem release consistency checks")
    parser.add_argument(
        "--strict",
        action="store_true",
