...
from auxmem.cli import build_parser
from auxmem.scaffold import parse_domains
from tests.helpers import REPO_ROOT, scaffold_auxmem
...
CANONICAL_GUIDANCE = [
    ...
    TEMPLATE / ".skills" / "auxmem-new-note" / "SKILL.md",
    TEMPLATE / ".skills" / "auxmem-setup-domains" / "SKILL.md",
    TEMPLATE / ".skills" / "auxmem-distill-seeds" / "SKILL.md",
    TEMPLATE / ".skills" / "auxmem-synthesize" / "SKILL.md",
    REPO_ROOT / "auxmem" / "importers" / "distill-seeds.md",
]
...
def test_canonical_guidance_points_to_config(path):
    ...
    assert ".scripts/auxmem.config.json" in text, f"{path} must reference the config file"
...
def test_cli_commands_documented_and_exist():
    ...
    for cmd in sub:
        assert f"## auxmem {cmd}" in usage, f"USAGE.md missing section for auxmem {cmd}"


def test_skills_exist_and_use_auxmem_prefix():
    skill_dirs = sorted(p for p in SKILLS_DIR.iterdir() if p.is_dir())
    assert skill_dirs, "no skills under template/.skills"
    for skill_dir in skill_dirs:
        assert skill_dir.name.startswith("auxmem-"), skill_dir.name
...
def test_scaffolded_agents_matches_onboarding(tmp_path):
    ...
    assert ".scripts/auxmem.config.json" in agents
    assert "docs/CONFLICTS.md" in agents
    for forbidden in FORBIDDEN_IN_CANONICAL:
        assert forbidden not in agents, forbidden
    config = (dest / ".scripts" / "auxmem.config.json").read_text(encoding="utf-8")
